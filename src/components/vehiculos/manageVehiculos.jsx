...
import {UpdateModalVehiculo} from './modales/modal'

const ManageVehiculos = ({Vehiculos}) => {
    ...
    useEffect(() => {
        if (Vehiculos && Vehiculos.length > 0) {
            setStatuses(Vehiculos.map(item => item.status));
        }
    }, [Vehiculos]);
    const handleSelect = (user) => {
        setSelectedVehiculo(user);
    ...
    if (Vehiculos === undefined || Vehiculos.length === 0) {
    ...
    return (
        ...
        />
          <section className="w-full">
            <div className="bg-blue-500 shadow-md rounded-bl-2xl rounded-tr-2xl w-fit"><h1 className="font-bold text-2xl p-4 text-gray-100">Gestionar Vehiculos</h1></div>
            <div className="p-4">
...
export default ManageVehiculos;