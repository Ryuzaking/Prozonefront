const ProductsCard = ({Datas}) => {
    ...
    return (
        ...
        <nav>
         ...
         <section className="w-full h-fit grid lg:grid-cols-4 md:grid-cols-2 sm:grid-cols-1 gap-8 p-8 justify-center items-center content-center">
         {Datas.map((data,index ) => (
            <div key={data.id}  className={`w-full text-center flex flex-col items-center justify-center bg-slate-100 rounded-3xl shadow-lg border-2 p-4 ${
                // Aplicar la clase para ocupar dos columnas en elementos específicos
