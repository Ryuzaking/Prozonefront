...
const ManageRutas = ({Rutas}) =>{
    ...
    const handleOpenRoutes = () => setOpen(true);
    const handleCloseRoutes = () => setOpen(false);
    useEffect(() => {
        if (Rutas && Rutas.length > 0) {
            setStatuses(Rutas.map(item => item.status));
        }
    }, [Rutas]);
    ...
    if (Rutas === undefined || Rutas.length === 0) {
    ...
    return (
        ...
        <UpdateModalRutas 
            open={open}
            handleOpen={handleOpenRoutes}
            handleClose={handleCloseRoutes}
